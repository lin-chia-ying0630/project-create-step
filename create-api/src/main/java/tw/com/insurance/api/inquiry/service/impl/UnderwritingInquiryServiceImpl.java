...
public class UnderwritingInquiryServiceImpl implements UnderwritingInquiryService {
	...
	public InquiryDetail find(String query) {
		...
		return new InquiryDetail(inquiryNo, text(row, "application_no"), policyNo, text(row, "underwriting_case_no"),
				...
				applicationStatus.newContractStageCode(), applicationStatus.newContractStageNameEn(),
				applicationStatus.newContractStageDescriptionZhTw(), applicationStatus.contractStatusCode(),
	...
	public InquiryPage findPage(int page, int pageSize, String sort) {
		...
		if (totalItems == 0)
			return new InquiryPage(List.of(), 0, query.page(), query.pageSize(), 0);
