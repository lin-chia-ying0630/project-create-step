...
新契約契約狀態包含：受理時資料庫值為 `NULL`，承保並生效後為 `01`（有效），人工核保負向決行為 `13`（拒保）、`14`（延期）或 `15`（取消），保單送達後十天猶豫期內的變更為 `26`（猶豫期變更）。核保審查作業將 `DC` 拒絕承保映射為階段 `NS`／契約狀態 `13`，`PO` 延期承保映射為 `DS`／`14`，`CN` 取消申請映射為 `CS`／`15`；原本代表承保完成的 `NS` 已遷移為 `AS`，避免與拒保完成衝突。`26` 屬新契約狀態，但不是核保結果，因此不列入核保審查結果選單。
