...
Northflank 的 `a-linlin/create-api` 是 `https://p01--create-web--kkj9gmg9xdcp.code.run/` 對應的公開測試 API，必須明確保留 `AUTH_MODE=demo`。變更 Northflank combined service 時，`runtimeEnvironment` 會整組取代，更新前必須先保留既有資料庫連線與加密密鑰設定，不得只送單一環境變數。部署後依序確認 `/api/auth/me` 回傳 `200` 與 `demo-user`、`/api/v1/reviews` 回傳 `200`，並以瀏覽器直接開啟 `/reviews`，確認沒有登入重新導向循環。前端路由守衛只有在 `/api/auth/me` 明確回傳 401 或 403 時導向統一入口；部署重啟期間的 502／503 或瀏覽器網路失敗必須保留目前 route，交由頁面顯示服務暫時無法使用，不得導回同源首頁形成重新導向迴圈。
...
審核及業務查詢清單的操作欄固定置於第一欄；其後前三個資料欄位使用共用可排序表頭，預設第一個資料欄位升冪。每筆資料固定顯示為一列且欄位不換行，欄位超過可用寬度時只在表格容器顯示水平捲軸。共用分頁元件預設每頁 10 筆，可選 10／20／50／100 筆；切換頁碼、筆數或排序都由後端重新查詢，不得對分頁後的前端資料再次排序。此契約套用於覆核工作台、核保審查、保單資料查詢、客戶建立、承保撤回、核保照會單及代碼定義查詢。覆核工作台可用完整覆核編號、客戶 ID、要保書號碼、正式保單號碼或業務鍵查詢待覆核案件；客戶 ID 透過要保作業關係人檔關聯，不解密或掃描覆核 payload。切換覆核功能分類時，分類條件同時套用於後端 count 與分頁 SQL，不以目前頁面的資料做前端過濾。
...
所有資料查詢清單預設每頁 10 筆，使用共用 `PageNavigator.vue` 提供 10、20、50、100 筆選項；頁碼與每頁筆數必須送至後端，SQL 以 `LIMIT`／`OFFSET` 只回傳當頁資料，前端不得對完整資料集或後端已分頁資料再次切片。代碼下拉選單為維持既有 typed API 相容性可使用完整有效選項端點，但 `/code-definitions` 查詢頁固定使用分頁端點，不得一次載入全部職業代碼。

客戶職業欄位使用共用 `CodeDefinitionAutocomplete.vue`；使用者輸入至少兩個字元後才搜尋，前端延遲 250ms 並取消舊請求，後端每次最多回傳前 20 筆符合的職業代碼。進入客戶建立頁時不得再預載全部 1,324 筆職業資料，且使用者必須從搜尋結果選取有效代碼。

資料建立與資料查詢使用不同 route 與側邊選單入口；例如 `/customers/new` 只顯示客戶建立表單，`/customers` 只顯示客戶查詢清單。

保單登打與覆核工作台的多區段導航統一使用 `SectionTabNavigator.vue`，採「頁次＋功能名稱」卡片頁籤；窄畫面由頁籤導航本身水平捲動。覆核功能的英文 operation type 與繁中顯示名稱以後端 `ReviewOperationType` 為唯一資料字典來源，`/api/v1/reviews/operation-types` 提供前端對照；`ReviewOperationTabs.vue` 只以 `v-for` 呈現 API 資料，覆核工作台不得再建立第二份對照陣列。
